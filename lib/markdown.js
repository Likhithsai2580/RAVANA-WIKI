...
/**
 * Normalize the content of a Mermaid diagram by applying various transformations.
 *
 * The function removes metadata sections, handles style directives, standardizes node syntax,
 * and ensures that the diagram type is declared. It also cleans up excessive whitespace
 * and formats the content for proper Mermaid rendering.
 *
 * @param content - The raw Mermaid content to be normalized.
 * @returns The normalized Mermaid content.
 */
 ...
 *
 * This function first parses the frontmatter from the provided content using the `matter` function.
 * It then processes the markdown content with `remark`, applying plugins for GFM and HTML conversion.
 * The resulting HTML is modified to replace mermaid code blocks with appropriate `<div>` elements,
 * handling various formats and ensuring proper normalization of diagram content.
 * Finally, it returns an object containing the processed HTML and the parsed frontmatter.
 *
 * @param {string} content - The markdown content to be processed.
 */
...
/**
 * Extracts Mermaid diagrams from the given content.
 *
 * This function uses a regular expression to find and extract Mermaid diagram definitions from the input string.
 * It processes each match to handle different formats, normalizes the diagram content, and stores the results
 * in an array, including the index of each diagram in the original content.
 *
 * @param {string} content - The input string containing potential Mermaid diagrams.
 */
